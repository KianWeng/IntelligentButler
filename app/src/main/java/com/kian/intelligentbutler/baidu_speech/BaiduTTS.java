...
public class BaiduTTS {
    ...
    public BaiduTTS(Context context, InitConfig initConfig) {
        this(context);
        init(initConfig);
    }


    protected BaiduTTS(Context context) {
        if (isInitied) {
    ...
    protected boolean init(InitConfig config) {
        ...
        if (isMix) {
            try {
                ...
                PPLog.e(TAG,"【error】:copy files from assets failed." + e.getMessage());
                return false;
            }
        }
        ...
        if (isMix) {
            ...
            PPLog.i(TAG,"离线资源路径：" + offlineResource.getTextFilename() + ":" + offlineResource.getModelFilename());
            ...
            if (!authInfo.isSuccess()) {
                ...
                PPLog.e(TAG,"鉴权失败 =" + errorMsg);
                isReady = false;
                return false;
            } else {
                //sendToUiThread("验证通过，离线正式授权文件存在。");
                PPLog.i(TAG,"验证通过，离线正式授权文件存在。");
            }
        }
        ...
        if (result != 0) {
            ...
            PPLog.e(TAG,"【error】initTts 初始化失败 + errorCode：" + result);
            isReady = false;
            return false;
        }
        // 此时可以调用 speak和synthesize方法
        //sendToUiThread(INIT_SUCCESS, "合成引擎初始化成功");
        PPLog.i(TAG,"合成引擎初始化成功");
        isReady = true;
        return true;
    }
    ...
    public int loadModel(String voiceType) {
        ...
        PPLog.i(TAG,"切换离线发音人成功。当前发音人：" + voiceStr);
        return res;
    }
